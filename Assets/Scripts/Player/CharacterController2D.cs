...
public class CharacterController2D : MonoBehaviour
{
    ...
    private void UpdateVelocity()
    {
        if (!data.GetDeadStatement())
        {
            ...
            {
                if (!FindObjectOfType<Bash>().getBash())
                {
    ...
    private void UpdateGravityScale()
    {
        ...
        if (!isOnGround)
        {
            ...
            else if (FindObjectOfType<Bash>().getBash())
            {
    ...
    private IEnumerator Dash()
    {
            ...
            canDash = false;
            isDashing = true;
        effecter.DoEffect(CharacterEffect.EffectType.flash, true);
        controllerRigidbody.velocity = new Vector2(-transform.localScale.x * dashingPower, 0f);
            tr.emitting = true;
            yield return new WaitForSeconds(dashTime);
        effecter.DoEffect(CharacterEffect.EffectType.flash, false);
        tr.emitting = false;
            isDashing = false;
